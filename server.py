...
TOKEN = "6183521273:AAGeaXYjP5kbSry5sxp-GuhyzQR4V9jh-p4"
...
def iniciar(update, context):
    update.message.reply_text("Seja bem-vindo!")
...
def treinar(update, context):
    update.message.reply_text("Modelo está sendo treinado!")
    model.compile(optimizer='adam', loss='sparse_categorical_crossentropy', metrics=['accuracy'])
    model.fit(x_train, y_train, epochs=10, validation_data=(x_test, y_test))
    model.save('cifar_classifier.model')
    update.message.reply_text("Pronto! Agora você pode mandar uma imagem")

        
def handle_message(update, context):
    update.message.reply_text("Por favor treine o modelo com uma imagem!")
...
def handle_photo(update, context):
    ...
    update.message.reply_text(f"Na imagem eu vejo um(a) {class_names[np.argmax(prediction)]}")
...
dp.add_handler(MessageHandler(filters._Photo, handle_photo))
